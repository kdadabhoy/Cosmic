# Run-WO08Render.ps1 — WO-08 (2D stability): one CosmicRenderTests / CosmicTests suite as an
# isolated, deadline-bounded acceptance child (R01–R07 and the retained golden check).
#
...
#   * points COSMIC_WO08_EVIDENCE_DIR at <Output>\captures so the cases' reviewable frames
#     (sentinel grids, RTT targets, the 641x359 capture, HDR read-back) land in evidence;
#   * for R07 sets COSMIC_WO08_PERF_OUT (the per-workload JSON) and passes --no-skip=true.
# The child's own doctest summary line is forwarded so the runner's minTests oracle sees it.
param(
    [Parameter(Mandatory=$true)][string]$Bin,
    [Parameter(Mandatory=$true)][string]$Output,
    [Parameter(Mandatory=$true)][string]$Suite,        # doctest --test-suite filter ("WO-08 R01")
    ...
    [int]$MinTests = 1,
    [int]$DeadlineSec = 120,
    [switch]$NoSkip,                                   # R07: run the skip(true) perf cases
    [string]$PerfOut = '',                             # R07: COSMIC_WO08_PERF_OUT base path
    [string]$CaseId = 'wo08')
...
foreach ($location in @($Bin, $Output)) {
    if (-not [IO.Path]::GetFullPath($location).StartsWith($repo, [StringComparison]::OrdinalIgnoreCase)) {
        throw 'WO-08 fixtures and artifacts must remain inside Cosmic'
    }
}
...
$caseObj = [pscustomobject]@{
    id = $CaseId; description = "WO-08 $Suite via $Exe"; tier = 'G';
