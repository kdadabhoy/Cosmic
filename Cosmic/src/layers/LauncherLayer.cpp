...
namespace Cosmic
{
	...
	void LauncherLayer::GenerateProjectTemplate(const std::string& baseDir, const std::string& projName)
	{
		...
		// 1. Error Check: Verify directory doesn't already exist
		...
		// 2. Run build.bat directly after creation asynchronously
		...
		{
			// Close handles so we don't leak process resources; it runs freely in its own console
	...
	void LauncherLayer::OnImGuiRender()
	{
		...
		if (ImGui::BeginTable("LauncherColumns", 2, ImGuiTableFlags_Resizable))
		{
			...
			// 3b. Failsafe Manual Scan Trigger Button
			...
			if (ImGui::BeginPopupModal("Project Creation Wizard", NULL, ImGuiWindowFlags_AlwaysAutoResize))
			{
				...
				static std::string modalError = ""; // Retains error locally until resolved or closed
				...
				if (ImGui::Button("Generate Template", ImVec2(140, 30)))
				{
					...
					if (!finalName.empty())
					{
						...
						// 1b. Error verification within UI state space before completing operations
						...
						else
						{
							...
							// 3a. Force internal project structural index refresh
							ScanForProjects();
