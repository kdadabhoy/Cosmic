...
namespace Cosmic
{
    ...
    /**
     * Represents a single vertex for line rendering.
     */
    struct LineVertex
    {
        glm::vec3 Position;
    ...
    /**
     * Represents a single vertex for quad/sprite rendering.
     */
    struct QuadVertex
    {
        glm::vec3 Position;
    ...
    /**
     * Represents a single vertex for hardware-accelerated procedural circles.
     */
    struct CircleVertex
    {
    ...
    /**
     * Internal state storage for the 2D Renderer.
     * Manages batching buffers, texture slots, and scene-wide uniforms.
     */
    ...
    /**
     * Initializes the 2D Renderer subsystems.
     * Allocates GPU buffers, loads core shaders, and prepares the batching staging area.
     */
    void Renderer2D::Init()
    {
		...
		s_Data.CircleVertexArray->SetIndexBuffer(quadIB); // Share the existing quad index layout buffer handle perfectly!
		s_Data.CircleShader = Shader::Create("assets/shaders/Circle.glsl");
    }

    /**
     * Shuts down the 2D Renderer.
     * Cleans up CPU staging buffers. GPU resources are handled by smart pointers.
     */
	...
	void Renderer2D::Flush()
	{
		...
		if (s_Data.QuadIndexCount != 0)
		{
			...
			// 1. Bind all utilized texture assets to their respective hardware units
	...
	void Renderer2D::DrawQuad(const glm::vec3& position, const glm::vec2& size, const Ref<Material>& material)
	{
		...
		// Dynamic State Tracker: If switching materials, submit and clean out previous batch context
		if (s_Data.CurrentMaterial != material)
			FlushAndReset();
		...
		// Resolve Texture hardware slot indexing dynamically
		float textureIndex = -1.0f;
		for (uint32_t i = 0; i < s_Data.TextureSlotIndex; i++)
		{
		...
		{
			s_Data.QuadVertexPtr->Position = transform * s_Data.QuadVertexPositions[i];
			s_Data.QuadVertexPtr->Color = color; // Pushed safely to vertex elements
			s_Data.QuadVertexPtr->TexCoord = texCoords[i];
			s_Data.QuadVertexPtr->TexIndex = textureIndex;
			s_Data.QuadVertexPtr->TilingFactor = 1.0f;
			s_Data.QuadVertexPtr++;
	...
	void Renderer2D::DrawCircle(const glm::vec3& position, const glm::vec2& size, const glm::vec4& color, float thickness, float fade)
	{
		...
		// 2. Generate standard 4-point quad boundary centered around the target position
		glm::mat4 transform = glm::translate(glm::mat4(1.0f), position)
			* glm::scale(glm::mat4(1.0f), { size.x, size.y, 1.0f });

		// Explicitly mapped bounds matching local coordinates for distance extraction
		...
		// 3. Populate 4 staging layout entries per circle primitive
		for (uint32_t i = 0; i < 4; i++)
		{
			s_Data.CircleVertexBufferPtr->WorldPosition = transform * s_Data.QuadVertexPositions[i];
			s_Data.CircleVertexBufferPtr->LocalPosition = localPositions[i];

			// 🟢 Normalize color if it originates from an integer-based ImGui widget
		...
		// Optional telemetry increment if tracking circle statistics:
		s_Data.Stats.QuadCount++; 
	}
