...
// PBRSkinned — Phase 20 (A2): the PBR surface shader for skinned meshes
// (Renderer3D::DrawMeshSkinned). The joint palette for every skinned draw of
// the frame lives in a std430 SSBO on binding 10 (Bindings::SkinningSsbo —
// claimed in BindingPoints.h by A2); u_SkinBase indexes this draw's palette.
// Joints arrive as floats (the engine's attribute pointers are float-typed;
// float32 is exact for joint indices) and round back to int here. Everything
// past the vertex stage matches PBR.glsl exactly — KEEP THE FRAGMENT STAGE IN
// SYNC WITH PBR.glsl (the PBRInstanced.glsl convention).
//
// Normal transform note: normals go through mat3(skin) + the engine's
// u_NormalMatrix and normalize — exact for the rigid/uniform-scale palettes
// character rigs produce (non-uniform bone scale would need a per-joint
// inverse-transpose; documented limitation, same as PBRInstanced).

layout(location = 0) in vec3 a_Position;
layout(location = 1) in vec3 a_Normal;
layout(location = 2) in vec2 a_TexCoord;
layout(location = 3) in vec4 a_Tangent;   // xyz = tangent, w = handedness sign
...
uniform int u_SkinBase;   // this draw's first palette entry
...
void main()
{
    // Weighted palette blend; renormalize so slightly lossy weights still sum 1.
    ...
    v_WorldNormal  = normalize(u_NormalMatrix * (skinN * a_Normal));
    v_WorldTangent = normalize(u_NormalMatrix * (skinN * a_Tangent.xyz));
