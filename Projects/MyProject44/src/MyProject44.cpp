...
namespace Workspace
{
	MyProject44::MyProject44()
		: Cosmic::Layer("MyProject44")
	{
	}

	// -------------------------------------------------------------------------
	void MyProject44::OnAttach()
	{
		CS_INFO("MyProject44: Attaching root manager layer.");

		// 1. Resolve asset paths through the VFS
		Cosmic::FileSystem::SetActiveProject("MyProject44");
		...
		{
			CS_WARN("MyProject44: Shader not found at '{0}'. Rendering layers will use fallback geometry.", shaderPath);
		}
		...
		CS_INFO("MyProject44: {} child layers attached. Fullscreen override active.", m_Modes.size());

		
	}

	// -------------------------------------------------------------------------
	void MyProject44::OnDetach()
	{
		for (auto& mode : m_Modes)
		{
			mode->OnDetach();
		...
		CS_INFO("MyProject44: Detached and cleaned up.");
	...
	// -------------------------------------------------------------------------
	void MyProject44::OnUpdate(float ts)
	{
	...
	// -------------------------------------------------------------------------
	void MyProject44::OnFixedUpdate(float deltaFixedTime)
	{
		if (m_Modes.empty()) return;
	...
	// -------------------------------------------------------------------------
	void MyProject44::OnImGuiRender()
	{
		if (m_Modes.empty()) return;
	...
	// -------------------------------------------------------------------------
	void MyProject44::OnEvent(Cosmic::Event& e)
	{
		if (m_Modes.empty()) return;
...
extern "C"
{
	...
	{
		return new Workspace::MyProject44();
	}
}