...
namespace Workspace
{
	...
	void aaaa::OnAttach()
	{
		CS_INFO("aaaa: Attaching root manager layer.");

		// 1. Resolve asset paths through the VFS
		Cosmic::FileSystem::SetActiveProject("aaaa");
		...
		{
			CS_WARN("aaaa: Shader not found at '{0}'. Rendering layers will use fallback geometry.", shaderPath);
		}
		...
		CS_INFO("aaaa: {} child layers attached. Fullscreen override active.", m_Modes.size());
	...
	void aaaa::OnDetach()
	{
		...
		CS_INFO("aaaa: Detached and cleaned up.");
	...
	// -------------------------------------------------------------------------
	void aaaa::OnFixedUpdate(float deltaFixedTime)
	{
		if (m_Modes.empty()) return;
	...
	// -------------------------------------------------------------------------
	void aaaa::OnEvent(Cosmic::Event& e)
	{
		if (m_Modes.empty()) return;
