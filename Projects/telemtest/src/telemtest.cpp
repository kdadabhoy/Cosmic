...
{
	telemtest::telemtest()
		: Cosmic::Layer("telemtest")
	{
	}

	// -------------------------------------------------------------------------
	void telemtest::OnAttach()
	{
		CS_INFO("telemtest: Attaching root manager layer.");

		// 1. Resolve asset paths through the VFS
		Cosmic::FileSystem::SetActiveProject("telemtest");
		...
		{
			CS_WARN("telemtest: Shader not found at '{0}'. Rendering layers will use fallback geometry.", shaderPath);
		}
		...
		CS_INFO("telemtest: {} child layers attached. Fullscreen override active.", m_Modes.size());

		
	}

	// -------------------------------------------------------------------------
	void telemtest::OnDetach()
	{
		for (auto& mode : m_Modes)
		{
			mode->OnDetach();
		...
		CS_INFO("telemtest: Detached and cleaned up.");
	...
	// -------------------------------------------------------------------------
	void telemtest::OnUpdate(float ts)
	{
	...
	// -------------------------------------------------------------------------
	void telemtest::OnFixedUpdate(float deltaFixedTime)
	{
		if (m_Modes.empty()) return;
	...
	// -------------------------------------------------------------------------
	void telemtest::OnEvent(Cosmic::Event& e)
	{
		if (m_Modes.empty()) return;
...
extern "C"
{
	...
	{
		return new Workspace::telemtest();
	}
}