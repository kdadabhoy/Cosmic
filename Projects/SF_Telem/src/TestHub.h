...
namespace Workspace
{
    ...
    public:
        ...
        // telemetry hub, so one connection persists across screen switches. The
        // root drives link.OnUpdate()/Shutdown(); this hub only polls when active.
        void Init(Cosmic::SerialLink* link);
        void Shutdown();
        void OnUpdate(float ts);     // clock + serial pump + rate windows

        // ---- Serial ----
        void DrawSerialPanel();      // "Serial Link" window
        bool Connected() const { return m_Link && m_Link->IsOpen(); }
    ...
    private:
        // --- Serial ---
        // Shared engine component (owned by the root manager). Set in Init();
        // never owned/destroyed here.
