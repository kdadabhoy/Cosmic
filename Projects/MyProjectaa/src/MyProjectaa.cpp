...
namespace Workspace
{
	MyProjectaa::MyProjectaa()
		: Cosmic::Layer("MyProjectaa")
	{
	}

	// -------------------------------------------------------------------------
	void MyProjectaa::OnAttach()
	{
		CS_INFO("MyProjectaa: Attaching root manager layer.");

		// 1. Resolve asset paths through the VFS
		Cosmic::FileSystem::SetActiveProject("MyProjectaa");
		...
		{
			CS_WARN("MyProjectaa: Shader not found at '{0}'. Rendering layers will use fallback geometry.", shaderPath);
		}
		...
		CS_INFO("MyProjectaa: {} child layers attached. Fullscreen override active.", m_Modes.size());

		
	}

	// -------------------------------------------------------------------------
	void MyProjectaa::OnDetach()
	{
		for (auto& mode : m_Modes)
		{
			mode->OnDetach();
		...
		CS_INFO("MyProjectaa: Detached and cleaned up.");
	...
	// -------------------------------------------------------------------------
	void MyProjectaa::OnUpdate(float ts)
	{
	...
	// -------------------------------------------------------------------------
	void MyProjectaa::OnFixedUpdate(float deltaFixedTime)
	{
		if (m_Modes.empty()) return;
	...
	// -------------------------------------------------------------------------
	void MyProjectaa::OnImGuiRender()
	{
		if (m_Modes.empty()) return;
	...
	// -------------------------------------------------------------------------
	void MyProjectaa::OnEvent(Cosmic::Event& e)
	{
		if (m_Modes.empty()) return;
...
extern "C"
{
	...
	{
		return new Workspace::MyProjectaa();
	}
}