...
namespace Showcase
{
	...
	void ShowcaseRunLayer::Reset()
	{
		...
		CS_INFO("ShowcaseRunLayer: Game reset.");
	}
	...
	void ShowcaseRunLayer::OnEvent(Cosmic::Event& e)
	{
		...
		dispatcher.Dispatch<Cosmic::KeyPressedEvent>(
			GLCORE_BIND_EVENT_FN(ShowcaseRunLayer::OnKeyPressed));
	}
	...
	void ShowcaseRunLayer::OnFixedUpdate(float fixedDt)
	{
		...
		for (auto& obs : m_Obstacles)
		{
			...
			{
				m_GameOver = true;
				CS_INFO("ShowcaseRunLayer: Game over! Score: {0:.0f}", dinoD.Score);
			}
		}
	...
	void ShowcaseRunLayer::OnRender()
	{
	...
	void ShowcaseRunLayer::OnImGuiRender()
	{
		...
		ImGui::Text("--- Runner Simulation ---");
		ImGui::Separator();

		ImGui::Text("Score:      %.0f", d.Score);
		ImGui::Text("High Score: %.0f", d.HighScore);
		ImGui::Text("Speed:      %.2fx", d.SpeedMultiplier);
		ImGui::Text("Obstacles:  %zu", m_Obstacles.size());
		ImGui::Spacing();

		if (m_GameOver)
		{
			ImGui::TextColored({ 1.0f, 0.3f, 0.3f, 1.0f }, "GAME OVER");
			ImGui::Text("Press Space / Up or click Reset to restart.");
		}
		else
		{
			ImGui::TextColored({ 0.3f, 1.0f, 0.3f, 1.0f }, "Running!");
			ImGui::Text("Space / Up = Jump    R = Reset");
		}
