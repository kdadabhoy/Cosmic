...
namespace Workspace
{
	...
	void test::OnAttach()
	{
		CS_INFO("test: Attaching root manager layer.");

		// 1. Resolve asset paths through the VFS
		Cosmic::FileSystem::SetActiveProject("test");
		...
		{
			CS_WARN("test: Shader not found at '{0}'. Rendering layers will use fallback geometry.", shaderPath);
		}
		...
		CS_INFO("test: {} child layers attached. Fullscreen override active.", m_Modes.size());
	...
	void test::OnDetach()
	{
		...
		CS_INFO("test: Detached and cleaned up.");
	...
	// -------------------------------------------------------------------------
	void test::OnFixedUpdate(float deltaFixedTime)
	{
		if (m_Modes.empty()) return;
	...
	// -------------------------------------------------------------------------
	void test::OnEvent(Cosmic::Event& e)
	{
		if (m_Modes.empty()) return;
