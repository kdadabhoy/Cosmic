...
| [Cameras & 2D Navigation](cameras-navigation.md) | Camera hierarchy, `Camera2DController`, orthographic controller, the editor orbit/fly rig, gizmos | SKELETON — D27 |
| [2D Renderer](rendering-2d.md) | Batching, texture slots, SDF circles, instancing, text | SKELETON — D28 |
| [Frame Pipeline & Post-Processing](rendering-pipeline.md) | SceneRenderer 2D spine, HDR, 2D light buffer, bloom/FXAA/tonemap/vignette | SKELETON — D29 |
| [ECS & Scenes](ecs-scene.md) | Entity-component model on entt, components, systems, scene rendering hooks | SKELETON — D31 |
| [Assets & Virtual File System](assets-vfs.md) | Asset cache, `engine://`/`project://`/`user://` schemes, shader preprocessing | SKELETON — D32 |
...
> work order; the one written row came out of Phase 29 W10 and is a complete document. The Status
> column is the only reliable signal of which is which. (2) **`main` is the 2D-only trunk
> (2026-09-18).** The explainers for the 3D systems — `rendering-3d`, `terrain`, `water`, `particles`,
> the 3D halves of `rendering-pipeline` and `cameras-navigation`, and `build-2d-3d-split` (the Phase 29
> two-configuration build, now superseded) — are parked under
> [`../parked-3d/systems/`](../parked-3d/README.md) (parked 3D) with their `STATUS: SKELETON` banners
> intact. No live explainer describes a 3D path as current.
