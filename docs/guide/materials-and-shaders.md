...
out vec4     v_Color;
out vec2     v_TexCoord;
flat out int v_TexIndex;            // the slot, flat — see "Index u_Textures with literals" below
...
**Index `u_Textures` with literals only.** `texture(u_Textures[int(v_TexIndex)], …)` looks natural
and NVIDIA/AMD Windows drivers accept it, but it is not portable GLSL: before 4.00 a sampler array
takes only a constant index, and from 4.00 on the index must be *dynamically uniform* — the same for
every fragment of the draw — which a batch that mixes textures never is. Mesa (VMware/VirtualBox
VMs, Intel and AMD on Linux) rejects the shader outright; elsewhere the result is undefined. The slot
therefore travels as a `flat int` and the fetch is a `switch` over literal indices, exactly what
`Texture.glsl` does (KI-82). `tests/check_gl_conformance.ps1` fails on any other form.

The engine's own `assets/shaders/Texture.glsl` is this shader without the extra uniforms — read it
when in doubt. Note it declares **no `u_Color`**: `Renderer2D`'s
