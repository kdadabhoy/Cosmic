...
| Fixture marker: `CS_TEST_FIXTURE()` (`scripting/ModuleMacros.h`) exporting `int CosmicTestFixture()`, invoked once by the nine `tests/*Fixture.cpp`; `static COSMIC_API std::vector<std::string> LauncherLayer::ScanForProjects(const std::vector<std::filesystem::path>& dirs)` (public; the member scan calls it with `{<CWD>/projects, <CWD>}`) skips a DLL exporting it — one trace line per skipped DLL, one info line `LauncherLayer: scan found N project(s): [...]` per distinct result (KI-77) | §3 | new | LH01-U (2 cases, CosmicTests both configs), LH01-W (`Run-UX03Launcher.ps1`, manifest `ux03-launcher`) | landed on `ux/03` (UX-03) |
| Sample discovery: `StarforgeApp::SampleInfo {Name, Kind, Description, SourcePath, Featured}`; `ListSamples()` = `templates/samples/*` + `<SdkDir()>/Projects/*` with a `project.cproj` (never Starforge), grouped App / Game / Other (raw `kind` key; absent = Other), PendulumLab featured + first with the thumbnail `Projects/Starforge/assets/editor/samples/PendulumLab.png`; `OpenSample` copies an SDK sample (skipping `build/`, `dist/`, `logs/`, `.starforge/`; `project.cproj` last) or scaffolds a template sample into `SamplePath(<Name>)` and seeds `.starforge/thumb.png`; the welcome popup (`playground_offered`, no new key) offers PendulumLab; New Project preselects App | §3 | changed (AP-03 E06 surface) | LH02 (E06 extended in `AP03AuthoringSelfTest.cpp`, manifest `ux03-launcher`) | landed on `ux/03` (UX-03) |
