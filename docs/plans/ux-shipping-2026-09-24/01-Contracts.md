...
| One-gizmo rule (UX-02) | `UiRectGizmo::Owns(e)` (RectTransform or Canvas) => `ViewportController::DrawGizmo` submits no transform gizmo; the strip's op / World-Local / snap / Frame chips draw disabled with `UI elements: use the rect gizmo` (slots + ids unchanged). Rect gizmo Move: the centre square (`UiRectGizmoMath::MoveHandleRect`, kHandleHalf around the centre) always captures the selected element, the rest of the rect only when it is topmost (`CapturesMove`). 2D rotation: `Gizmo::ApplyModel(TransformComponent&, const glm::mat4&, bool mode2D)` (pure), `Gizmo::Manipulate(..., bool mode2D = false)` — 2D writes `Rotation.z` (deg), keeps X/Y, `UseQuatRotation` false, `RotationQuat` untouched, Z ring only. Pick: a selected world sprite's bounds are tested before the UI hit-test; a press there keeps the selection and (Move / Universal) body-drags it, one `CommitTransform` per gesture | editor UX + engine API (Gizmo.h) | ED01 (unit + `ux02-editor`), KI-72..75 | landed on `ux/02` |
| UI Active semantics (UX-02) | `UiSystem::VisitUi` skips a node whose `TagComponent::Active` is false and its whole subtree (canvas roots keep the `IsActiveInHierarchy` check), so `Render`, `Update`, `HitTest`, `CollectHostedPanels` and the editor's rect gizmo ignore it; no new field | engine behaviour (runtime + editor) | ED02, KI-76 | landed on `ux/02` |
| Flow-usage hits (UX-02) | `SourceHit` gains `Kind` (`SourceHitKind::Source` / `Flow`), `State`, `Target` (as written; `push <State>` for a push), `Signal`, `FlowVfs`, `StateIndex`, `TransitionIndex`, `FlowLine()` = `Flow: <State> —<signal>→ <Target>`; `SourceLocator::ForSignal` appends one Flow hit per startup-flow transition whose `On` equals the signal AFTER the src hits (path = the .cflow, line 0, state/target in `Reason`); `FlowHitsForSignal`, `StartupFlowRel` public. Inspector Signal rows (UiButton / UiSlider / UiToggle) and the viewport context menu show each line with **Open in flow editor** (`StarforgeApp::OpenFlowHit`: the Screens-panel document path + `FlowEditor::HarnessSelect`). `ResolveLogicSource` keeps opening a Source hit | editor API + UX | ED03 (unit + `ux02-editor`), E08 unchanged | landed on `ux/02` |
| Scenes list (UX-02) | `SourceLocator::ProjectScenes(root)`: every `scenes/**/*.cscene` as `project://scenes/<rel>`, recursive, case-insensitive sort, `.bak` and other extensions never; Screens ▸ Scenes (start scene = the flow start state's scene, else `startup_scene`; open scene highlighted; double-click -> the prompt-guarded open; right-click -> Reveal) and File ▸ Open Scene draw that one list | editor UX | ED04 (unit + `ux02-editor`) | landed on `ux/02` |
| Prefs keys (UX-02) | `user://starforge/editor.toml`: `autosave_minutes` (kept; whole minutes 1-60, default 5; a legacy float is rounded, a legacy value <= 0 reads as `autosave_enabled = false` when that key is absent), `autosave_enabled` (bool, true), `prompt_unsaved` (bool, true); `Prefs::LoadSettingsFrom(path)` / `SaveSettingsTo(s, path)` (LoadSettings / SaveSettings wrap them). Edit ▸ Preferences… modal (the three controls, the autosave folder + Reveal); status bar `autosaved HH:MM`; Save / Discard / Cancel around the USER commands New Scene, Open Scene (menu, Screens panel), Open Project (Recent), Close Project, Exit to Launcher; the window close writes an autosave copy in `OnDetach` (no vetoable close exists) | user file format + editor UX | ED05 (unit + `ux02-editor` incl. the out-of-process OnDetach copy) | landed on `ux/02` |
