...
| KI-2 | Coverage gap | SerialLink connected-state (auto-reconnect at `Cosmic/src/serial/SerialLink.cpp:67`, one-shot `ConsumeJustConnected`) is unreachable headlessly — stated in `tests/test_serial_lifecycle.cpp:14-20`. Needs an injectable transport seam before WO-05 connected-state tests can run. **WO-05a (2026-09-17) delivered the exact seam spec: `evidence/WO-05a/transport-seam-spec.md`** — inject only at the four OS calls (`CreateFileA`/DCB, overlapped `ReadFile`+wait, `WriteFile`, `CancelIoEx`/`CloseHandle`) + `SerialLink` pass-through ctor; state machine stays real. | WO-04 (seam), WO-05 (use) |
