...
| KI-2 | **Seam built (WO-04, 2026-09-17); full use = WO-05** | Was: SerialLink connected-state (auto-reconnect at `Cosmic/src/serial/SerialLink.cpp:67`, one-shot `ConsumeJustConnected`) unreachable headlessly (`tests/test_serial_lifecycle.cpp:14-20`). **WO-04 built the injectable transport seam per `evidence/WO-05a/transport-seam-spec.md`**: `serial/ISerialTransport.h` + `Win32SerialTransport` (shipping default, verbatim OS calls) + test-only `tests/FakeSerialTransport.h`; injected ONLY at the four OS calls + `List()`; both `std::thread`s, `m_Abandon`, the stop event, every `State` transition and SerialLink auto-reconnect stay real. Connected state now reachable under test — `tests/test_serial_seam.cpp` drives Connect+PushBytes->IsReceiving/ConsumeJustConnected/Poll-exact-bytes, drop->Failed->reconnect, and the KI-4 teardown join (observable, ~1.2 s). Inert in shipping (Win32 default; 0 Fake symbols in Cosmic.dll; not packaged); `test_serial_lifecycle` still green. Evidence: `evidence/WO-04/`. Remaining: WO-05 authors the full connected-state matrix + un-skips the WO-05a H1/H3 seats + fixes KI-4. | WO-04 (seam ✅), WO-05 (use) |
