...
| M9 | Animation / timeline | Sprite + skeletal clips, crossfade, `TimelineState`; the wall-clock scheduler in `Application` dispatch is now driven by N01/N02 over the injected `IFrameClock` (WO-10): 30/60/144-Hz + irregular schedules, speeds 0/0.25/1/4, pause/resume, the 0.25 s stall, plugin-local vs global scaling, origins 0/2 h/24 h, the non-finite / negative policy, a 2-h drift run. **Known limitation (KI-56):** `Layer::GetLocalTime()` is a float accumulator (−16.9 s after 2 h). | C01, N01–N02, X01 | `test_animation.cpp`, `test_sprite_animation.cpp`, `test_crossfade.cpp`, `test_timeline_state.cpp`, `test_wo10_n01_dispatch.cpp`, `test_wo10_n02_clock.cpp` (+ `WO10ClockFixture`) | verified (WO-10) + known-limitation |
...
| M11 | Sim / math | RK4/semi-implicit integrators, filters, PCG RNG, noise — deterministic, same-build/seed scope. WO-10 re-asserts the catalog's exact parameters (N03), pins 1,000,000 PCG32 outputs against an independent oracle and two-process byte-identical streams (N04), and measures the float telemetry path against the double F-TRAJECTORY reference. `IntegrateRK4`'s step arguments are `float` (a double state integrates with a float-rounded `dt/6`, ~1e-8 relative floor — recorded, not a defect). | N03–N04, X01 | `test_integrators.cpp`, `test_filters.cpp`, `test_random.cpp`, `test_noise.cpp`, `test_wo10_n03_numerics.cpp`, `test_wo10_n04_determinism.cpp` | verified (WO-10) |
