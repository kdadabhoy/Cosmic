...
### KI-84 — Timing-bound CosmicTests fail under CPU load on the 8-vCPU campaign VM: WO-05 T03 (serial matrix; 2 s waits, 250 ms / 2000 ms bars) intermittently in both configs even on re-runs, WO-09 C05 (2819 ms and 2013 ms against a 2000 ms deadline) and E10 (file watcher) under load
...
- Anchor (at `353948b`): `tests/test_wo05_matrix.cpp:64` (`Until(... State::Open)`), `:131`, `:255`, `:277` (`REQUIRE(rig.fake->WaitOpenEntered())`), `:218` (`CHECK(ms <= 2000)`), `:281` (`CHECK(duration <= 250ms)`) — the `WO-05 T03*` cases at `:102`, `:247`, `:271`, `:292`; `tests/test_wo09_c05_json.cpp:218` (`maxMs < 2000.0`, truncation case `:204`) and `:437` (`st->maxMs <= deadlineMs`, parser-fuzz case `:404`); `tests/test_filewatcher.cpp:83-84` (`E10: a new file in the watched tree is reported`, `:61`).
- Repro: on the VMware VM (8 vCPU, llvmpipe) run `CosmicTests.exe` (either config) while another lane builds with `--parallel 4` or runs CosmicRenderTests / editor self-tests: T03 fails intermittently (e.g. `CHECK( 10155 <= 2000 )`, `REQUIRE( rig.fake->WaitOpenEntered() )`, `396936600ns <= 250ms`), C05 misses its 2000 ms deadline (2819.86 ms, 2013.2 ms), E10 reports no change. Re-run alone: C05 and E10 pass; T03 still fails intermittently (4/4 failed once in Debug with no other process at start); control: `main` `d815252` Release `-tc="WO-05 T03*"` passed 4/4 alone.
- Failing-before: `docs/plans/ux-shipping-2026-09-24/evidence/UX-V0/ki84-excerpts.txt` (the UX-01 lane's Phase A runs, copied verbatim, plus UX-V0's own two full runs that passed 523/0/14).
