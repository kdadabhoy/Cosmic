...
- Failing-before: `docs/plans/app-platform-2026-09-18/evidence/KI-64-65/before-stale-*.png` (the 00:05 package on the real display), `failing-before-ki64-excerpts.txt` (the new unit cases run against the pre-`68e482a` mapping on an isolated patch).
...
- Failing-before: `docs/plans/app-platform-2026-09-18/evidence/KI-64-65/failing-before-ki65-excerpts.txt` (the KI-65 unit case against the unfixed `Update`).
...
- Disposition: `UiSystem::SliderKnobRect(rect, orientation, knobSizePx, min, max, value)` (pure) is the single knob geometry; `DrawSlider` draws it and `Update` grabs by `rect ∪ knob rect` (the track stays grabbable everywhere it was). Fixed in this lane.
