...
## UX & Shipping findings (2026-09-24) — UX-03 (launcher and samples)

### KI-77 — The Cosmic Launcher lists the test-fixture DLLs (`AP01ServiceFixture`, `WO07*Fixture`, `WO10ClockFixture`, ...) as runnable projects under "Engine demos & tools": `ScanForProjects` accepts any DLL beside the exe that exports `CreatePluginLayer`, and `tests/CMakeLists.txt` builds the fixtures into the same `build/Runtime/<cfg>` folder
- Status: Confirmed defect (wrong behaviour: a click on a fixture tile loads a test DLL as a project). Owner WO: UX-03.
- Anchor: `Cosmic/src/layers/LauncherLayer.cpp:582-634` at `6021822` — `ScanForProjects` scans `<CWD>/projects` then `<CWD>` (`:590-591`) and keeps every DLL whose `GetProcAddress(hMod, "CreatePluginLayer")` is non-null (`:619-623`); the list is drawn under "Engine demos & tools" (`:365`, `:382-398`). `tests/CMakeLists.txt:176-254` puts ten fixture DLLs in `${COSMIC_SDK_DIR}/build/Runtime/$<CONFIG>`.
- Repro: configure with `-DCOSMIC_BUILD_TESTS=ON`, build, start `build\Runtime\Debug\CosmicApp.exe` with no arguments and CWD = its folder: the fixture names appear in the demo list beside `SF_Telem`.
- Failing-before: `docs/plans/ux-shipping-2026-09-24/evidence/UX-03/failing-before-excerpts.txt` (LH01-U and LH01-W against the unfixed scan loop, isolated patch).
- Regression: `tests/test_launcher_scan.cpp` cases `UX-03 LH01 ...` (CosmicTests both configs); LH01-W `tests/acceptance/fixtures/Run-UX03Launcher.ps1` (manifest `ux03-launcher`).
- Disposition: open — fix in UX-03 (an explicit `CS_TEST_FIXTURE()` export in every fixture, skipped by the scan; no name matching).

### KI-78 — The homescreen project card draws `.starforge/thumb.png` upside down (and squashed into its 88 px band): `DrawProjectCard` uses the UV pair (0,0)-(1,1) for a texture the engine loads V-flipped
- Status: Confirmed defect (visual; latent since project thumbnails exist — every card whose project the editor saved shows its thumbnail mirrored; UX-03's seeded PendulumLab thumbnail makes it visible on a fresh profile). Found by UX-03 in passing. Owner WO: none yet (the line is outside UX-03's §10 row); proposed for UX-Q1.
- Anchor: `Projects/Starforge/src/StarforgeApp.cpp:2631-2632` at `6021822` (`dl->AddImage(..., ImVec2(0, 0), ImVec2(1, 1))` in `DrawProjectCard`) vs `OpenGLTexture::OpenGLTexture(path)` calling `stbi_set_flip_vertically_on_load(1)` (`Cosmic/src/platform/OpenGL/OpenGLTexture.cpp:79`); `CaptureThumbnail` writes a top-down PNG (`OpenGLFrameBuffer::ReadPixels` flips rows to a top-left origin). The brand logo (`StarforgeApp.cpp:2428-2429`) and the Content Browser (`ContentBrowserPanel.cpp:445-446`, `:956-957`) draw the same kind of texture with the flipped pair (0,1)-(1,0).
- Repro: put a project with a `.starforge/thumb.png` in the library (any project saved in the editor, or PendulumLab opened from the samples) and show the homescreen: the card's thumbnail is vertically mirrored while the featured-sample thumbnail above it — the same PNG, drawn with the flipped pair — is upright. Capture: `docs/plans/ux-shipping-2026-09-24/evidence/UX-03/ki78-project-card-upside-down.png` (Debug, lane build).
- Regression: none yet (a self-test readback of the card band compared with the PNG's top rows would do).
- Disposition: open — the fix is the flipped UV pair (plus an aspect-preserving fit) in `DrawProjectCard`; not applied by UX-03 (file range outside its ownership).
