...
- Disposition: open (UX-V0 fixes it: a `flat` integer slot varying and a `switch` over the 32 literal slots, `#version 450 core`, identical sampler/UV/tiling/colour maths).
...
- Regression: to write (VM02) — a broken shader file fed through the production loader must produce one clear error and a clean non-zero exit (or a continued run without the dependent feature), never an access violation.
- Disposition: open (UX-V0).
