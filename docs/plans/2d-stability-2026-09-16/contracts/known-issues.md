...
## fix/player-pointer findings (2026-09-20) — the PendulumLab pointer reports (App Platform packet)

### KI-64 — The two pointer defects reported from the exported PendulumLab on 2026-09-20 (windowed: every click lands one button row low; fullscreen: a slider knob grabs only when the cursor is above it) are KI-61 still shipping: the qualified package `dist/PendulumLab` (staged 00:05 from `fa1223a`) predates the fix `68e482a` (02:18), and that fix had no headless regression — the standalone pointer mapping lived inline in `PlayerLayer::UpdateUI`, reachable only by the env-gated editor harness on one machine state
- Status: Confirmed (stale shipped binary + a coverage gap; NO new mapping defect exists at `ff41ea5` — the same reproduction passes on a package rebuilt from it). Owner WO: `fix/player-pointer` lane (this entry).
- Anchor: `Cosmic/src/layers/PlayerLayer.cpp` `PlayerLayer::UpdateUI` — the mapping `Input::GetMouseScreenPosition() - app.GetViewportPos()` scaled by `framebuffer / GetViewportSize()`, written inline with no pure seam; `C:\dev\Cosmic\dist\PendulumLab\Cosmic.dll` 2026-09-20 00:04:25 (AP-Q1 Y02 staging) vs `68e482a` 2026-09-20 02:18:25.
- Repro (Kaden's display: 2560x1440 primary, `GetDpiForSystem()` = 96, i.e. 100 % scaling): run the 00:05 package maximized (client 2560x1392). Home screen: press with the cursor 16 px UNDER the Start button (window y 963; Settings' authored rect is y 1007-1081) -> the Settings screen opens; the cursor exactly on Settings hovers nothing. F11 (borderless fullscreen 2560x1441, menu bar hidden, the Viewport dock tab bar stays): press ON the Gravity knob -> no drag; press 21 px ABOVE the knob -> it grabs (9.807 -> 15.262). The offsets are exactly the chrome above the Viewport dock's content: menu bar + tab bar = 54 px windowed, tab bar alone = 27 px fullscreen — KI-61's window-client-vs-frame mismatch in both of its chrome configurations. The same package rebuilt from `ff41ea5` (Release, `installer/Stage-AppPackage.ps1`): cursor on Settings hovers and opens Settings; fullscreen press on the knob centre drags it; a press 30 px above the knob (outside its rect) does not.
- Failing-before: `docs/plans/app-platform-2026-09-18/evidence/KI-64-65/before-stale-*.png` (the 00:05 package on the real display), `failing-before-ki64-65-excerpts.txt` (the new unit cases run against the pre-`68e482a` mapping on an isolated patch; 4/4 cases fail), `after-fresh-*.png` / `after-fixed-*.png` for the passing side.
- Regression: `tests/test_ui_widgets.cpp` suite `KI-64/65 pointer mapping` — `KI-64 MapPointerToCanvas: chrome offset, framebuffer scale, identity, degenerate sizes`, `KI-64 buttons: a pointer mapped from screen space hits the button it points at under a chrome offset, a 1.5x framebuffer scale and a letterbox band; the pre-fix window-client pointer hits the row below`, `KI-64 slider knobs: the same three mappings grab the knob centre and refuse 1 px above the knob's top where the rect is not` (headless, CosmicTests both configs).
- Disposition: the mapping is one pure function, `UiSystem::MapPointerToCanvas(pointerScreen, frameScreenPos, frameScreenSize, framebufferSize)`, that `PlayerLayer::UpdateUI` calls and the unit cases test directly; `COSMIC_UI_DEBUG_POINTER=1` makes `UiSystem::Render` stroke every interactable's hit rect and cross-hair the resolved pointer so one screenshot shows an offset numerically; PendulumLab restaged from this lane and re-verified windowed + fullscreen on the display (`after-fresh-*.png`). PROCESS: every package staged before `68e482a` carries KI-61 — `C:\dev\Cosmic\dist\PendulumLab` (the Y02 / showcase package) and any SF_Telem package must be restaged after this lane merges; the K02/Y02 evidence on main was produced from the stale layout and should be re-run then.

### KI-65 — `UiSystem::Update` hit-tests a slider by its RectTransform rect, not by the knob it draws: a knob larger than the track rect (`KnobSize` x scale > the rect's thickness) cannot be grabbed by its visible overhang (latent; found while investigating KI-64 — NOT the cause of the fullscreen symptom, whose sliders' 18 px knobs sit inside 24 px rects)
- Status: Confirmed defect (input, latent for PendulumLab; real for any authored slider with a thin track and a big knob). Owner WO: `fix/player-pointer` lane (this entry).
- Anchor: `Cosmic/src/scene/ui/UiSystem.cpp` `UiSystem::Update` topmost-hit loop (`it->Rect.Contains(pointer.Position)` for every interactable alike) vs `DrawSlider` (`Renderer2D::DrawQuad(knobCenter, { knob, knob })` with `knob = max(2, KnobSize * scale)` centred on the track at the value's position) — two geometries with no shared source.
- Repro: slider rect 200x8 px (horizontal), `KnobSize` 24, value at 50 %: press 10 px above the rect's centre line — on the drawn knob, outside the rect — no drag begins; the same press inside the 8 px band drags. A vertical slider shows the same across its width.
...
- Disposition: fixed in UX-01 `1fb0310` (branch `ux/01`, registered in `3f717f0`; FE04 PASS both configs at the 1920x1080 window on the pre-VM PC (canvas 852x551 px docked at Center, 4/4 state nodes inside); on the campaign VM (display 1718x920, the OS clamps the window to 1738x940) FE04 is ENVIRONMENT_BLOCKED: canvas 737x454 px, docked, focused, 4/4 nodes inside — the 1920x1080 re-check is HOST-VERIFY) — first-use size 1100x680, `SetNextWindowFocus()` on the frame after `Open()` adds or re-focuses a document, the Assets, Telemetry and Level presets dock "Editors" at `DockPort::Center` (tabbed with the Viewport; Animation keeps its existing `BottomCenter` dock), `FlowEditor` centres the canvas once after the first layout, the inspector column collapses from the toolbar.
...
- Status: Confirmed defect (rejected by Mesa llvmpipe on the campaign VM; undefined behaviour on every driver for `QuadInstance.glsl`). Owner WO: UX-V0.
- Anchor (at `353948b`): `Cosmic/assets/shaders/Texture.glsl:2,27` (`#version 330 core` — in GLSL 1.30-3.30 a sampler array may only be indexed by a constant expression) and `:36,40` (`uniform sampler2D u_Textures[32]` indexed by `int(v_TexIndex)`); `Cosmic/assets/shaders/QuadInstance.glsl:56,61-65` (the same array indexed by `int(v_TexIndex)` under `#version 450 core` — legal syntax, but GLSL 4.50 §4.1.7 requires the index to be dynamically uniform and one batch/instanced draw mixes slots, so the result is undefined). Same pattern in the render-test fixtures `tests/render/fixtures/wo08/wo08_tint_quad.glsl:40,45` and `wo08_tint_quad_instance.glsl:44,50-52`, and in the developer guide's custom-shader example `docs/guide/materials-and-shaders.md:328-334`. No other `.glsl` in the repo (28 files, vendored dependencies have none) declares a sampler array.
- Repro: on the VM (`OpenGL 4.5 — llvmpipe (LLVM 13.0.1, 256 bits)`) start `build\Runtime\Release\Starforge.exe` from a `353948b` build: the log shows `0:17(18): error: sampler arrays indexed with non-constant expressions are forbidden in GLSL 1.30 and later` for `assets/shaders/Texture.glsl`, then `Shader::Create ... Returning nullptr` (the crash that follows is KI-83). NVIDIA/AMD Windows drivers accept the shader, which is why no real-GPU run ever showed it.
- Failing-before: `docs/plans/ux-shipping-2026-09-24/evidence/UX-V0/failing-before-excerpts.txt`; `tests/check_gl_conformance.ps1`'s sampler-index pass (added by UX-V0) exits 1 on the unfixed tree naming the four files.
- Regression: `tests/check_gl_conformance.ps1` second pass (every `*.glsl` in the repo, vendored exempt: exit 1 when a `uniform sampler*` array is indexed by anything but an integer literal, naming `file:line`; runs in CI's "GL conformance audit" step); VM01 (no shader compile error in the Starforge / CosmicApp / CosmicRenderTests logs under llvmpipe).
- Disposition: fixed in UX-V0. `Texture.glsl` (now `#version 450 core`), `QuadInstance.glsl` and both `wo08_tint_quad*` fixtures carry the slot as a `flat int` (`int(a_TexIndex)` in the vertex stage — the same truncation, exact per quad) and fetch through `SampleSlot(slot, uv)`, a `switch` over the literal indices 0-31 (out of range: `vec4(1.0)`, i.e. colour only — Renderer2D never emits one; `QuadInstance.glsl` keeps its `0 <= index < 32` guard). Same sampler, same `uv * tiling`, same colour multiply, the 32-slot contract unchanged. The guide's custom batch-shader example now uses the same pattern and says why. Passing-after: the checker exits 0; the llvmpipe logs of Starforge / CosmicApp / CosmicRenderTests show no shader error (`evidence/UX-V0/`); the real-GPU golden run is HOST-VERIFY (`evidence/UX-V0/HOST-VERIFY.md`).

### KI-83 — A failed engine shader (`Shader::Create` returns nullptr) crashes the process with an access violation (0xC0000005) instead of failing with a clear message
- Status: Confirmed defect (crash at start-up on any driver that rejects `Texture.glsl`; latent null dereferences for `Line.glsl` / `Circle.glsl` at the first line/circle flush). Owner WO: UX-V0.
- Anchor (at `353948b`): `Cosmic/src/renderer/Renderer2D.cpp:237-240` — `CS_CORE_ASSERT(s_Data.TextureShader, ...)` is compiled out in every configuration (`Cosmic/src/core/Core.h:61-82`: `CS_ENABLE_ASSERTS` needs `CS_DEBUG`/`GLCORE_DEBUG`, which nothing defines), so `s_Data.TextureShader->Bind()` dereferences null; `:682` (`s_Data.LineShader->Bind()`) and `:698-700` (`activeCircleShader->Bind()`) have no null guard (a failed `Line.glsl` / `Circle.glsl` crashes at the first flush that holds lines / circles); `Cosmic/src/platform/OpenGL/OpenGLShader.h:123` — `m_RendererID` has no initializer, so on a compile/link failure `IsValid()` reads an indeterminate value (a Debug heap fill of `0xCDCDCDCD` makes a failed shader look valid and `Shader::Create` returns it); the failure log (`Shader::Create: compilation or link failure for '<path>'. Returning nullptr.`) names neither the GL renderer/version nor the compiler's error, which sits dozens of lines above under the preprocessed dump.
- Repro: on the VM start `Starforge.exe` (Release, `353948b`): exit code `-1073741819` (0xC0000005) about 4 s after launch, the log ending at the `Returning nullptr` line; `CosmicRenderTests.exe` (Release) exits with the same code during `Renderer2D::Init`, before any test case runs.
- Failing-before: `docs/plans/ux-shipping-2026-09-24/evidence/UX-V0/failing-before-excerpts.txt`.
- Regression: `tests/render/render_ux_v0_shader_failure.cpp` suite `UX-V0 VM02` (CosmicRenderTests, GL context) with the real broken fixture `tests/render/fixtures/ux_v0/ux_v0_broken.glsl`: `Shader::Create` on it (nullptr + one error line with path, renderer/version and first compiler error), `Renderer2D::Init` with `COSMIC_SHADER_OVERRIDE` mapping `Texture.glsl` (Init false, frame runs, quads skipped) and `Line.glsl` + `Circle.glsl` (Init true, quads draw, lines/circles skipped), and `CosmicApp.exe` started with the Texture override (exit 2, "could not start" on stderr, never 0xC0000005).
- Disposition: fixed in UX-V0, per call site: **fatal, clean** — `Texture.glsl` (Renderer2D's batch shader): `Renderer2D::Init` returns false with the reason (`GetInitError`), `Application` stops the boot after `Renderer::Init` with one CRITICAL line, `Run()` returns at once, `Runtime/Main.cpp` prints `Fatal: <app> could not start: ...` to stderr (a message box when a GUI build has no stderr; `COSMIC_NO_FATAL_DIALOG=1` suppresses it), deletes the app (normal `Shutdown`) and exits `Application::StartupFailureExitCode` (2). **Continue without the feature** — `Line.glsl`, `Circle.glsl` (`Flush` now skips a batch whose shader is null; quads skip too if a material's shader is null), `Text.glsl` (already skipped), `CircleInstance.glsl` / `QuadInstance.glsl` (the instanced verbs already return early), `Light2D.glsl` / `BlitCopy.glsl` (`Light2DRenderer::Ensure` returns false, lights skipped), `Tonemap.glsl` and the five effect shaders (`PostProcessStack` already guards each pass), `Launcher.glsl` (plain clear colour), `AssetLibrary::GetShader` (returns nullptr to its caller; `BuildMaterial` returns nullptr). `Shader::Create` logs ONE error line naming the path, `renderer '<GL_RENDERER>', OpenGL '<GL_VERSION>'` and the compiler's first error line (also `Shader::GetLastCreateError()`); `OpenGLShader::m_RendererID` is initialised to 0 (no more Debug "valid" failed shaders); an unreadable file / a file with no stages fails before any GL work. The VM02 start-up case caught a second crash on the new failure path during development — `Application::OnWindowResize` dereferenced the not-yet-created framebuffer when the Window's teardown (chrome restore) reported a resize: exit `0xC000041D` (an access violation inside the WndProc) — now guarded.

### KI-84 — Timing-bound CosmicTests fail under CPU load on the 8-vCPU campaign VM: WO-05 T03 (serial matrix; 2 s waits, 250 ms / 2000 ms bars) intermittently in both configs even on re-runs, WO-09 C05 (2819 ms and 2013 ms against a 2000 ms deadline), WO-09 C02 "maximum map" (14455 ms against a 10 s save + load bar) and E10 (file watcher) under load
- Status: Confirmed defect (test robustness: the bars are wall-clock and assume an unloaded host; not a product defect as far as known). Registered by UX-V0 at the orchestrator's request (registration only, no fix). Owner WO: UX-H1 (hardening).
- Anchor (at `353948b`): `tests/test_wo05_matrix.cpp:64` (`Until(... State::Open)`), `:131`, `:255`, `:277` (`REQUIRE(rig.fake->WaitOpenEntered())`), `:218` (`CHECK(ms <= 2000)`), `:281` (`CHECK(duration <= 250ms)`) — the `WO-05 T03*` cases at `:102`, `:247`, `:271`, `:292`; `tests/test_wo09_c05_json.cpp:218` (`maxMs < 2000.0`, truncation case `:204`) and `:437` (`st->maxMs <= deadlineMs`, parser-fuzz case `:404`); `tests/test_wo09_c02_tilemap.cpp:131` (`msSave + msLoad < 10000.0`, the maximum-map case `:74`); `tests/test_filewatcher.cpp:83-84` (`E10: a new file in the watched tree is reported`, `:61`).
- Repro: on the VMware VM (8 vCPU, llvmpipe) run `CosmicTests.exe` (either config) while another lane builds with `--parallel 4` or runs CosmicRenderTests / editor self-tests: T03 fails intermittently (e.g. `CHECK( 10155 <= 2000 )`, `REQUIRE( rig.fake->WaitOpenEntered() )`, `396936600ns <= 250ms`), C05 misses its 2000 ms deadline (2819.86 ms, 2013.2 ms), C02's 1,048,576-cell save + load takes 14454.8 ms against 10000 ms (UX-02's Debug run; 3566 ms alone), E10 reports no change. Re-run alone: C05, C02 and E10 pass; T03 still fails intermittently (4/4 failed once in Debug with no other process at start); control: `main` `d815252` Release `-tc="WO-05 T03*"` passed 4/4 alone.
