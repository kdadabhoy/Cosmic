...
| DG-7 | flowchart LR | 3D submission: DrawMesh → frustum cull → sort key → auto-instance detect → flush (opaque F2B, transparent B2F) | ✅ **built D54** in [`../guide/rendering-3d.md`](../../parked-3d/guide/rendering-3d.md#dg-7--what-one-submission-actually-does); reference/rendering-3d + systems/rendering-3d reuse it | Renderer3D.cpp, RenderQueue.h |
| DG-8 | flowchart TD | SceneRenderer pass graph incl. read/write targets: shadow → coverage → reflection → refraction → main HDR → water → particles → post chain → present | ✅ **built D55** in [`../guide/lighting-and-environment.md`](../../guide/lighting-2d.md#dg-8--the-pass-graph); reference+systems rendering-pipeline reuse it | SceneRenderer.cpp, frame-lifecycle.md |
